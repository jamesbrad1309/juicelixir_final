...
gulp.task('vendorcss', function(){
	return gulp.src($.mainBowerFiles())
	.pipe($.filter(['**/*.scss', '**/*.sass', '**/*.css', '**/*.less']))
	.pipe($.order([
		'bootstrap.css',
		'*'
	]))
...
// Build task
gulp.task('build', ['make-bootstrap-css', 'theme-style', 'fontTask', 'imageTask', 'vendorjs', 'vendorcss', 'projectjs', 'projectcss', 'phpTask']);
...
gulp.task('watch', ['browser-sync'], function(){
	//bootstrap config
	gulp.watch('./config.json',['make-bootstrap-css','vendorcss','projectcss']);

	gulp.watch(config.JS_DIR + '/**/*.js', ['vendorjs', 'projectjs']);
	gulp.watch(config.CSS_DIR + '/**/*.sass', ['vendorcss', 'projectcss', 'theme-style']);
